...
function speedupClass(speedup: number | null): string {
...
// Leaderboard panel
// --------------------------------------------------------------------------- //
function renderLeaderboard(boards: LeaderboardBoard[], baseline: string): void {
  ...
  for (const board of boards) {
    ...
    const head = el("tr");
    ["#", "variant", "wins", "configs", "geomean speedup"].forEach((h) =>
      head.appendChild(el("th", undefined, h)),
    );
    table.appendChild(head);
    board.entries.forEach((e, i) => {
      const tr = el("tr");
      if (e.variant === baseline) tr.classList.add("baseline-row");
...
// --------------------------------------------------------------------------- //
let resultsCache: ResultRow[] = [];

function renderResults(rows: ResultRow[]): void {
  resultsCache = rows;
  const hwSel = document.getElementById("hw-filter") as HTMLSelectElement;
  ...
  if (hardwares.includes(current) || current === "") hwSel.value = current;

  const onlyImprovements =
    (document.getElementById("only-wins") as HTMLInputElement).checked;
  drawResultsTable(hwSel.value, onlyImprovements);
}

function drawResultsTable(hw: string, onlyWins: boolean): void {
  const root = document.getElementById("results-body")!;
  clear(root);
  let rows = resultsCache;
  if (hw) rows = rows.filter((r) => r.hardware === hw);
  if (onlyWins) rows = rows.filter((r) => r.speedup !== null && r.speedup > 1);
  ...
  const table = el("table");
  const head = el("tr");
  ["hardware", "n", "case", "batch", "variant", "block", "mean ms", "baseline ms", "speedup", "ok"].forEach(
  ...
  for (const r of rows) {
    const tr = el("tr");
    if (r.is_baseline) tr.classList.add("baseline-row");
...
// Agents panel
// --------------------------------------------------------------------------- //
function staleClass(lastUpdate?: string): string {
...
function renderAgents(agents: Agent[]): void {
  ...
  if (agents.length === 0) {
    root.appendChild(
      el("p", "empty", "No agents. Run dashboard/seed_demo.py or start a worker."),
    );
    return;
  }
  for (const a of agents) {
    ...
    if (staleClass(a.last_update)) badge.classList.add("stale");
    header.appendChild(badge);
    card.appendChild(header);

    const task = a.task_summary || a.task || "";
    if (task) card.appendChild(el("div", "agent-task", task));

    const meta = el("div", "meta");
    if (a.current_job) meta.appendChild(el("span", "chip", `job ${a.current_job}`));
    if (a.cuda_visible_devices !== undefined)
      meta.appendChild(el("span", "chip", `gpu ${a.cuda_visible_devices}`));
    if (a.pid) meta.appendChild(el("span", "chip", `pid ${a.pid}`));
    if (a.inbox_messages) meta.appendChild(el("span", "chip", `📬 ${a.inbox_messages}`));
    if (a.last_update) {
...
// Queue panel
// --------------------------------------------------------------------------- //
const QUEUE_COLUMNS: Array<keyof QueuePayload> = [
  "pending",
  "running",
  "done",
  "failed",
...
function renderQueue(queue: QueuePayload): void {
...
function jobCard(job: Job): HTMLElement {
  ...
  if (job.priority) head.appendChild(el("span", "chip", `p${job.priority}`));
  card.appendChild(head);
  if (job.hardware) card.appendChild(el("div", "chip hw-chip", job.hardware));
  if (job.command) card.appendChild(el("code", "cmd", job.command));
  const meta = el("div", "meta");
  if (job.passed) meta.appendChild(el("span", `chip ${job.passed === "true" ? "good" : "bad"}`, `passed ${job.passed}`));
  if (job.runtime_s) meta.appendChild(el("span", "chip", `${job.runtime_s}s`));
  if (Array.isArray(job.depends_on) && job.depends_on.length)
    meta.appendChild(el("span", "chip", `deps ${job.depends_on.length}`));
  if (meta.childNodes.length) card.appendChild(meta);
  if (job.next_action) card.appendChild(el("div", "next", `→ ${job.next_action}`));
  return card;
}
...
async function refresh(): Promise<void> {
  ...
  try {
    ...
    const data: AllPayload = await res.json();
...
function init(): void {
  (document.getElementById("hw-filter") as HTMLSelectElement).addEventListener(
    "change",
    () => refresh(),
  );
  (document.getElementById("only-wins") as HTMLInputElement).addEventListener(
    "change",
    () => refresh(),
  );
